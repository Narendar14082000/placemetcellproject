...
module.exports.create = async(req, res) => {
    try {
        ...
        console.error("Error in creating interview:", err);
        // req.flash("error", "Failed to schedule interview. Please try again.");
        return res.redirect("back");
    }
};
...
module.exports.deallocate = async(req, res) => {
    try {
        ...
        if (interview) {
            ...
            await Interview.findOneAndUpdate({ _id: interviewId }, { $pull: { students: { student: studentId } } });

            // remove interview from student's schema using interview's company
            await Student.findOneAndUpdate({ _id: studentId }, { $pull: { interviews: { company: interview.company } } });
            return res.redirect("back");
        }
        return res.redirect("back");
    } catch (err) {
